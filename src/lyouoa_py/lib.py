...
def parser_loglist(html_text) -> list:
...
def find_ower(loglist: list, jiudian_name: str, fj_type: str) -> dict:
    row = [x for x in filter(lambda r: re.search(jiudian_name, r['操作']),
        loglist) if re.search(fj_type, x['操作'])]
    if len(row) >= 1:
        return {
            'ower': row[0].get("用户"),
            'opt_time': row[0].get("时间")
        }
    else:
        return {}
...
class LyouoaClient():
    ...
    def __init__(self, company_code: str, host: str = "http://vip.lyouoa.com") -> None:
    ...
    def get_tanhao(self, company_code: str):
        ...
        headers=self.headers, cookies=self.cookies, params={})
